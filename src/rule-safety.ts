...
export function assertAllowedActions(actions: readonly RuleActionInput[] | NormalizedRule["actions"]): void {
  if (actions.length !== 1) throw new FireflyError("FIREFLY_RULE_UNSAFE", "A pending rule must contain exactly one allowed action.");
  for (const action of actions) {
    if (action.type !== "set_category") throw new FireflyError("FIREFLY_RULE_UNSAFE", `Rule action ${JSON.stringify(action.type)} is not allowed.`);
    if (typeof action.value !== "string" || action.value.trim() === "" || action.value.length > 1024 || ("active" in action && action.active !== true)) throw new FireflyError("FIREFLY_RULE_UNSAFE", "set_category requires an active non-empty existing category name.");
  }
}
