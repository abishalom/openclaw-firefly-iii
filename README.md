...
- Confirmation requires the reviewed `proposalDigest`, verifies the persisted inactive rule by GET, then uses a minimal activation update and authoritative GET readback. A failed or ambiguous activation is rolled back to inactive only when ownership can still be verified, without overwriting concurrent semantic edits; otherwise the tool reports `FIREFLY_ACTIVATION_UNCERTAIN` for operator inspection.
